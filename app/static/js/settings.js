...
if (createKeyBtn) {
  createKeyBtn.addEventListener("click", async () => {
    ...
    const resp = await fetch("/settings/api-keys/create", {
      method: "POST",
    ...
    row.innerHTML = `
      <td class="font-monospace small">${data.prefix}…</td>
      <td>${data.label || "—"}</td>
  ...
  document.querySelector("#keys-table tbody").addEventListener("click", async (e) => {
    ...
    if (btn.classList.contains("revoke-btn")) {
      ...
      const resp = await fetch(`/settings/api-keys/${id}/revoke`, { method: "POST" });
    ...
    if (btn.classList.contains("delete-btn")) {
      ...
      const resp = await fetch(`/settings/api-keys/${id}/delete`, { method: "POST" });
